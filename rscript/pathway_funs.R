...
metabolomicsPathwayAnalysis <- function(
  ...
  for (i in seq_along(col.test)) {
    ...
    if ('MSEA' %in% analyses) {
      ...
      ## MSEA
      cat('\n\n')
      cat(paste0(title.prefix, '## Metabolite Set Enrichment Analysis', '\n\n'))
    ...
    if ('MetPA' %in% analyses) {
      ...
      ## MetPA
      cat('\n\n')
      cat(paste0(title.prefix, '## Metabolic Pathway Analysis', '\n\n'))
...
msetora <- function(
  ...
  #         las = 1, cex.name = 0.5, space = c(0.75, 0.75), main = title)
  # minP <- min(pvals)
  # maxP <- max(pvals)
  # medP <- (minP + maxP)/2
  # axs.args <- list(at = c(minP, medP, maxP), labels = format(c(maxP, medP, minP), scientific = T, digit = 1), tick = F)
  # fields::image.plot(legend.only = TRUE, zlim = c(minP, maxP), col = ht.col, axis.args = axs.args, legend.shrink = 0.4)
  # mtext(line=2, side=3, "P value", outer=F,adj = 1.5,padj =12)
  # par(op)
...
pathora <- function(cmpd.vec, lib.type, 
  ...
  if (!exists('metpa')) {
    destfile <- paste(lib.type, ".rda", sep = "")
...
idMap <- function(ids) {
  ...
  id_type <- lapply(ids, function(x) {
    if (stringr::str_detect(tolower(x), '^hmdb')) {
      return(c(x, x, 'hmdb'))
    }
    
    if (stringr::str_detect(tolower(x), '^kegg')) {
      return(c(x, stringr::str_extract(x, 'C[0-9]+$'), 'kegg'))
    }
    
    if (stringr::str_detect(tolower(x), '^metlin')) {
      return(c(x, stringr::str_extract(x, '[0-9]+$'), 'metlin'))
    }
    
    if (stringr::str_detect(tolower(x), '^pubchem')) {
      return(c(x, stringr::str_extract(x, '[0-9]+$'), 'pubchem'))
    }
  ...
  colnames(id_type) <- c('original', 'Query', 'type')
  ids <- id_type[, 'Query']
  id.type <- id_type[, 'type']
  ...
  for (type.i in unique(id.type)) {
    if (type.i %in% c('name', 'hmdb', 'pubchem', 'chebi', 'kegg', 'metlin')) {
      ...
      map.table <- mSet$dataSet$map.table
      
      testthat::expect_equal(ids[ind] %>% unname, map.table[, 'Query'] %>% unname)
      map.table <- cbind(type = type.i, map.table)
      
      if (is.null(out)) {
        out <- map.table
      } else {
        out <- rbind(out, map.table)
      }
      

    } else {
      warning(ifelse(type.i == '', 'Whitespace', type.i), 
              ' is not supported. Use hmdb, pubchem, chebi, kegg, metlin.')
    }
  }
  
  out <- merge(data.frame(id_type), data.frame(out), by = c('type', 'Query'))
